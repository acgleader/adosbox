...
class AccelerometerReader implements SensorEventListener {
	...
	public AccelerometerReader(Activity context) {
		System.out.println("libSDL: accelerometer start required: "
				+ String.valueOf(Globals.UseAccelerometerAsArrowKeys));
		reconfig(context);
	}

	public synchronized void reconfig(Context context) {
		if (Globals.UseAccelerometerAsArrowKeys) {
			_manager = (SensorManager) context
					.getSystemService(Context.SENSOR_SERVICE);
			if (_manager != null) {
				System.out.println("libSDL: starting accelerometer");
				// TODO: orientation allows for 3rd axis - azimuth, but it will
				// be way too hard to the user
				// if( ! _manager.registerListener(this,
				// _manager.getDefaultSensor(Sensor.TYPE_ORIENTATION),
	...
	public synchronized void onSensorChanged(SensorEvent event) {
		...
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			if (Globals.HorizontalOrientation)
				nativeAccelerometer(event.values[1], -event.values[0],
						event.values[2]);
			else
				nativeAccelerometer(event.values[0], event.values[1],
						event.values[2]); // TODO: not tested!
		} else {
			if (Globals.HorizontalOrientation)
				nativeOrientation(event.values[1], -event.values[2],
						event.values[0]);
			else
				nativeOrientation(event.values[2], event.values[1],
						event.values[0]);
		}
